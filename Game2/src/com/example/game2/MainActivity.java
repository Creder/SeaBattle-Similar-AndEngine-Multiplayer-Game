...
public class MainActivity extends SimpleBaseGameActivity
{
	...
	private void RightRandomInsertShip()
	{
		...
		while (
			 ...
			 if(ShipList[T]==0) T--;
			 //получаем случайные координаты для установки корабля
			 X=Rand(0,width); //координата по X
			 Y=Rand(0,width); //координата по Y
			 //получаем случайное направление корабля
			 D=Rand(0,1);
	...
	private boolean LInsert_ship(int coordX, int coordY, int TypeShip, int Direction)
	{
		...
		int LenghShip=TypeShip-1;
		 switch (Direction) 
		 {
		 //проверяем длину по X
		 case 0 : 
		 LenghShip+=coordX;
		 break;
		 //проверяем длину по Y
		 case 1 : 
		 LenghShip-=coordX;
		 break;
		 }
		 if ((LenghShip > 0) && (LenghShip < 10)) {
			 int TestX = coordx;
			 int TestY = coordy;
			 //начальные координаты для проверки 
			 //перебираем остальные палубы
			...
			/* if(!canSetShip(TestX,TestY)) {
			 return false;
				 ...
				 if(Direction==0) TestX = TestX+((int)cellSize);//значит по X
				 if(Direction==1) TestY = TestY+((int)cellSize);//значит по Y
			 }
			 //если мы прошли все проверки ставим корабль 
			 TestX = coordx;
			 TestY = coordy;
			 for (int P=0; P<TypeShip; P++) 
			 {
			 ...
			 //все в порядке говорим что установили корабль
			 return true;
			 } else
			 return false;
	}
